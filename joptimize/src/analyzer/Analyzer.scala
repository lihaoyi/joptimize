...
class Analyzer(entrypoints: Seq[MethodSig],
  ...
  def walkMethod(originalSig: MethodSig,
    ...
    if (callStack.contains(originalSig -> inferredArgs)) dummyResult
    else{
      frontend.apply(originalSig, log.method(originalSig)) match{
        case None => dummyResult
        case Some(program) =>
          log.global().println(
            "  " * callStack.length +
